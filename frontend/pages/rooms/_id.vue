...
import { initOptions, options, confOptions } from "~/utils/jitsi";
  ...
  setup(_, { root }) {
    ...
    function _onConnectionSuccess() {
      ...
      app.$room = app.$connection.initJitsiConference(
        roomRef.value.name.toLowerCase(),
        confOptions,
      );
    ...
    function _onRemoteTrackAdd(track: any) {
      ...
      if (!app.$remoteTracks.value[id]) {
        root.$set(app.$remoteTracks.value, id, ref({}));
      }
      ...
      if (type === "video") {
        root.$set(app.$remoteTracks.value[id].value, "video", track);
        consola.log(
      ...
      if (type === "audio") {
        root.$set(app.$remoteTracks.value[id].value, "audio", track);
        consola.log(
    ...
    function _onRemoteTrackRemove(track: any) {
      ...
      if (app.$remoteTracks.value[id]) {
        ...
        if (type === "video") {
          root.$delete(app.$remoteTracks.value[id].value, "video");
        }
        if (type === "audio") {
          root.$delete(app.$remoteTracks.value[id].value, "audio");
        }
      }
    }
